...
void ChangePalettePeriodically()
{
  ...
  if( lastSecond != secondHand) {
    ...
    if( secondHand ==  0)  { targetPalette = CRGBPalette16( bl,bl,bl,bl, bl,bl,bl,bl, w,w,b,b, b,b,bl,bl); }
...
void mapNoiseToLEDsUsingPalette2()
{
  ...
  for(int i = 0; i < kMatrixWidth; i++) {
    for(int j = 0; j < kMatrixHeight; j++) {
      ...
      uint8_t index = (noise[j][i]/2)+(band[1]/2+(band[6]/2))*160;
      uint8_t bri =   (noise[i][j]/2)+(band[1]/2+(band[6]/2))*160;
